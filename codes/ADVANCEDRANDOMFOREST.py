...
if model_selection_loop:
    ...
    for split, train_test_split in enumerate(train_test_zip):
        ...
        for target in targets:
            model_name = f"model_{target}_ARF"
            print(f"model: {model_name}")

            # train a model on the training split (and save it for future use)
            split_model_name = f"model_{target}_split{split+1}cv{cv}downsample{downsample_cross_val}_ARF"
            split_model = load_model(split_model_name)
            if not split_model:
                print(f"training model: {model_name}")
                split_model = RandomForestRegressor(n_estimators = 100, random_state = 42)
    ...
    for target in targets:
        gc.collect()
        model_name = f"model_{target}_downsample{downsample_full_train}_ARF"
