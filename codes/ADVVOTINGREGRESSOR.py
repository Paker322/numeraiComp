...
if model_selection_loop:
    ...
    for split, train_test_split in enumerate(train_test_zip):
        ...
        for target in targets:
            model_name = f"model_advKarma_deneme_{target}"
            print(f"model: {model_name}")

            # train a model on the training split (and save it for future use)
            split_model_name = f"model_adeneme_{target}_split{split+1}cv{cv}downsample{downsample_cross_val}"
            split_model = load_model(split_model_name)
            if not split_model:
                print(f"training model: {model_name}")

                lgbbestgdbt = lgb.LGBMRegressor(bagging_fraction=0.5, boosting_type='rf', class_weight=None,
                                              colsample_bytree=0.1, feature_fraction=0.5,
                                              importance_type='split', learning_rate=0.01, max_depth=5,
                                              min_child_samples=20, min_child_weight=0.001, min_split_gain=0.0,
                                              n_estimators=2000, n_jobs=-1, num_leaves=2 ** 5,
            ...
            training_data.loc[test_split_index, f"preds_{model_name}"] = \
                split_model.predict(training_data.loc[test_split_index, feature_cols])
    ...
    for target in targets:
        gc.collect()
        model_name = f"model_deneme_{target}_downsample{downsample_full_train}"
        ...
        if not model:
            ...
            # Defining the ensemble model
            model = lgbbestgdbt
            model.fit(training_data.loc[downsampled_train_split_index, feature_cols],
                            training_data.loc[downsampled_train_split_index,
                                              [target]])
...
validation_data["prediction"].to_csv(f"prediction_files/validation_predictions_deneme_{current_round}.csv", index=True)
tournament_data["prediction"].to_csv(f"prediction_files/tournament_predictions_deneme_{current_round}.csv", index=True)
